...
class Prefix(nn.Module):
    def __init__(self, args, config):
        ...
        if isinstance(config, RobertaConfig):
            ...
            self.num_transformer_submodules = 3

        self.match_n_embd = self.n_embd // self.match_n_head
        self.attn_bn = args.attn_bn
        self.embedding = torch.nn.Embedding(self.attn_bn * self.num_transformer_submodules,
                                            self.match_n_embd)
        self.transform = torch.nn.Sequential(
            torch.nn.Linear(self.match_n_embd, config.mid_dim),
            torch.nn.Tanh(),
            torch.nn.Linear(config.mid_dim, self.match_n_layer * 2 * self.match_n_embd),
        )
        self.prompt_tokens = torch.arange(
            self.attn_bn * self.num_transformer_submodules
        ).long()

    def forward(self, bsz, nsamples=1, device="gpu"):
        prompt_tokens = self.prompt_tokens.unsqueeze(0).expand(bsz, -1).to(device)
        past_key_values = self.embedding(prompt_tokens)
        past_key_values = self.transform(past_key_values)

        past_key_values = past_key_values.reshape(self.match_n_layer, self.num_transformer_submodules, 2, bsz,
                                                  self.attn_bn, -1)
        result = []
        keys = ['self', 'encoder_decoder', 'encoder']
        for i in range(self.match_n_layer):
            key_val = past_key_values[i]
            temp_dict = {}
            for j in range(self.num_transformer_submodules):
                temp_dict[keys[j]] = {
                    "prev_key": key_val[j][0].contiguous().view(bsz, -1,self.match_n_embd).repeat(self.match_n_head, 1, 1),
                    "prev_value": key_val[j][1].contiguous().view(bsz, -1,self.match_n_embd).repeat(self.match_n_head, 1, 1),
                    "prev_key_padding_mask": torch.zeros(bsz, self.attn_bn).to(key_val.device)
                }
...
class PrefixCrossAttn(nn.Module):
    ...
    def forward(self, bsz, nsamples=1, device="gpu"):
        ...
        for i, key_val in enumerate(past_key_values):
            ...
            temp_dict['self'] = {
                "prev_key": key_val2[0].contiguous().view(bsz * self.match_n_head, -1, self.match_n_embd),
    ...
    def forward(self, bsz, nsamples=1, device="cuda"):
        ...
        for i, (enc_attn_k, enc_attn_v, dec_self_attn_k, dec_self_attn_v, dec_xattn_k, dec_xattn_v) in \
                enumerate(zip(self.encoder_attn_key, self.encoder_attn_value, self.decoder_self_attn_key,
                              self.decoder_self_attn_value, self.decoder_cross_attn_key,
                              self.decoder_cross_attn_value)):
            temp_dict = {'self': {"prev_key": self._shape(dec_self_attn_k(input_tokens), bsz),
                         ...
                         'encoder_decoder': {"prev_key": self._shape(dec_xattn_k(input_tokens), bsz),
                                             ...
                                             "prev_key_padding_mask": torch.zeros(bsz, self.attn_bn).to(device)
                                             # bsz, attn_bn
                                             },
                         'encoder': {"prev_key": self._shape(enc_attn_k(input_tokens_enc), old_bsz),
                                     ...
                                     "prev_key_padding_mask": torch.zeros(old_bsz, self.attn_bn).to(device)
                                     # bsz, attn_bn
                                     },
                         }
            result.append(temp_dict)
...
class Adapter_Layer(nn.Module):
    def __init__(self,
        ...
        self.down_proj = nn.Linear(self.n_embd, int(self.down_size))
        self.non_linear_func = nn.ReLU()
        self.up_proj = nn.Linear(int(self.down_size), self.n_embd)
