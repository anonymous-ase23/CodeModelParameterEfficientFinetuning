...
class PLBartAttention(nn.Module):
    ...
    def forward(
        ...
        if hasattr(self.config, "attn_mode") and prefix_state is not None and 'prefix' in self.attn_mode:
            # legacy
        ...
        if hasattr(self.config, "attn_mode") and self.config.attn_mode == 'adapter' and self.config.attn_option == "parallel":
                cross_attn_output = self.ef_attn_adapter(hidden_states, add_residual=False)

        src_len = key_states.size(1)
        attn_weights = torch.bmm(query_states, key_states.transpose(1, 2))

        if hasattr(self.config, "attn_mode") and self.config.attn_mode != "none":
        ...
        if hasattr(self.config, "attn_mode") and self.config.attn_mode != "none" and self.config.attn_composition == "gate_add":
        ...
        # the Houlsby config
        if hasattr(self.config, "attn_mode") and self.config.attn_mode == "adapter" and self.config.attn_option == "sequential":
...
class PLBartEncoderLayer(nn.Module):
    def __init__(self, config: PLBartConfig, layer_id=None):
        super().__init__()
        self.config = config
        ...
        if hasattr(self.config, "attn_mode") and config.ffn_mode == 'lora':
        ...
        if hasattr(self.config, "attn_mode") and config.ffn_mode == 'adapter':
            # import pdb; pdb.set_trace()
    ...
    def forward(
        ...
        if hasattr(self.config, "attn_mode") and 'adapter' in self.config.ffn_mode and self.config.ffn_option == 'parallel':
        ...
        if hasattr(self.config, "attn_mode") and 'adapter' in self.config.ffn_mode:
...
class PLBartDecoderLayer(nn.Module):
    def __init__(self, config: PLBartConfig, layer_id=None):
        ...
        if hasattr(config, "attn_mode") and config.ffn_mode == 'lora':
        ...
        self.final_layer_norm = nn.LayerNorm(self.embed_dim)
        if hasattr(config, "attn_mode") and config.ffn_mode == 'adapter':
            # import pdb; pdb.set_trace()
            self.ef_ffn_adapter = Adapter_Layer(self.config,
                                                dropout=self.dropout,
                                                bottleneck=config.ffn_bn,
...
class PLBartEncoder(PLBartPreTrainedModel):
    ...
    def __init__(self, config: PLBartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        if hasattr(self.config, "attn_mode") and config.attn_mode == "prompt_tuning":
    ...
    def forward(
        ...
        if hasattr(self.config, "attn_mode") and self.config.attn_mode == "prompt_tuning":
        ...
        if hasattr(self.config, "attn_mode") and self.config.attn_mode == 'dlisa':
    ...
    def forward(
        ...
        Args:
            ...
            return_dict (`bool`, *optional*):
                Whether or not to return a [`~utils.ModelOutput`] instead of a plain tuple.
        """
        if hasattr(self.config, "attn_mode") and self.config.attn_mode == 'dlisa' and prefix_state is not None and hasattr(prefix_state, 'biases') and prefix_state.biases is not None:
...
class PLBartModel(PLBartPreTrainedModel):
    ...
    def forward(
        ...
        if hasattr(self.config, "attn_mode") and self.config.attn_mode == "prompt_tuning" and encoder_outputs is None:
            # training time
            attention_mask = torch.cat(
